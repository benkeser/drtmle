...
abs_text: |
  ...
  targeted minimum loss-based estimators) are widely used in causal inference
  to estimate and draw inference about the average effect of a treatment. As an
  ...
  estimated using maximum likelihood and parametric models. Confidence
  intervals and p-values may be computed based on standard asymptotic results,
  such as the central limit theorem, the delta method, and the nonparametric
  bootstrap. However, in high-dimensional settings maximum likelihood estimation
  often breaks down and standard procedures no longer yield correct inference.
...
when estimating treatment effects. However, facilitating proper inference (i.e.,
...
inference techniques, including both G-computation (GCOMP) estimators
...
utilizing adaptive estimators. In particular, with rare exception, if
cross-validation is used to select tuning parameters for the nuisance
parameters, estimates of the population-level effect will generally be too
biased to achieve standard $n^{1/2}$-rate asymptotics. Instead, *undersmoothing*
must be used to select tuning parameters to control the bias
[@undersmoothed_hal]. However, this can prove quite challenging in practice and
...
targeted minimum loss-based (TML) estimation [@vdlrubin:2006:ijb]. The
...
enjoys standard $n^{1/2}$ asymptotics. These asymptotic results also yield
closed form standard error estimates, which can serve as the basis for
...
recent work has shown that the double robustness property does not necessarily
extend to the inferential properties of these estimators when adaptive
estimators of the PS and/or OR are used
[@vdl:2014:ijb; @benkeser:2017:biometrika]. Instead, valid inference requires
consistent estimation of *both* the PS *and* the OR. @vdl:2014:ijb proposed new
TML estimators that have an asymptotic normal sampling distribution with
estimable variance under consistent estimation of *either* the PS *or* the OR,
paving the way for inference that is doubly-robust. @benkeser:2017:biometrika
...
parameters. The \texttt{drtmle} package was motivated by the need for a
...
the \texttt{drtmle} package are capable of estimating population-level effects for
...
usage of the \texttt{drtmle} package.
...
\bar{Q}_n(a,w) dQ_{W,n}(w)$. Typically, the empirical distribution function of
...
estimators generally lack a basis for statistical inference.
...
finite samples [@porter:2011:ijb]. We refer readers to @tlbook:2011 for
more about TMLE.
...
technique. We demonstrate each of these approaches in turn on a  simulated
...
To avoid overfitting initial regressions, one can use cross-validated 
estimates of the nuisance
regressions. Theoretically, the use of cross-validated regression weakens the
assumptions necessary to achieve asymptotic normality [@zheng:2010:working].
This is true of the regular TML and AIPTW estimators, as well as of the TML
estimator with doubly-robust inference. The `drtmle()` function implements
cross-validated estimation of the regression parameters through the `cvFolds`
