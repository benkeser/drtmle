...
omitted by users interested only in implementation. For simplicity, we focus
this discussion on the standard TMLE/AIPTW estimators. The ideas generalize
immediately to the DRTMLE estimator, but the formulas are more complex.

Reported standard error estimates are based on an estimate the asymptotic
variance of the estimators. This variance can be characterized by the estimators
influence function.  For example, let $\psi_{n}(1)$ be an AIPTW estimate of
$\psi_0(1)$. Under regularity conditions,
\begin{equation*}
...
variance of $D_i = D^{\star}(\bar{Q}_n, g_n, Q_{W,n})(O_i)$,
\begin{equation*}
  \sigma^2_n = \frac{1}{n} \sum_{i=1}^n \left\{ D_i - \frac{1}{n} \sum_{j=1}^n
  D_j \right\}^2 \ .
\end{equation*}
Thus, a natural estimate of the standard error of $\psi_{n}(1)$ is
$\sigma_n / n^{1/2}$. This idea generalizes naturally to estimating the
