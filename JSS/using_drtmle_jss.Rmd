...
parameters. The \pkg{drtmle} package was motivated by the need for a
...
\pkg{drtmle} implements an IPTW estimator that overcomes shortcomings of
...
the \pkg{drtmle} package are capable of estimating population-level effects for
...
usage of the \pkg{drtmle} package.
...
To understand the variable names available for R-OR, recall from (\ref{eqn:qbarr}) that the parameter
...
formula specifies a model for (\ref{eqn:gr_bivariate}) and assumes data with two columns, `"Qn"` and `"gn"`.
...
\pkg{SuperLearner} package. We refer readers to the documentation of the
...
generalized linear model (via the function `SL.glm` from \pkg{SuperLearner}) and
...
\pkg{earth} package [@earthPackage]).
...
The \pkg{drtmle} package also allows users to pass in their own functions for
...
(see `SuperLearner::write.SL.template()`). The \pkg{drtmle} package includes
...
estimator using the \pkg{np} package [@npPackage]. Below we show the source code
for this function.
...
In \pkg{drtmle}, cross-validated standard error estimates can be obtained by
...
when super learner is used to estimate the OR and PS. In \pkg{drtmle}, we give
...
The \pkg{drtmle} package contains methods that compute doubly-robust confidence
...
\pkg{drtmle} is fully equipped to handle treatments with an arbitrary number of
...
users of \pkg{drtmle} have access to the large number of binary regression
...
carried out using futures (with the API provided by the \pkg{future} R package),
...
are computed sequentially, as per the defaults of the \pkg{future} package. It
is left to the user to specify appropriate system-specific future backends that
may better suit their problem (e.g., using the \pkg{future.batchtools} package
...
\pkg{future.batchtools} package for further information.
...
The \pkg{drtmle} package was designed to provide a user-friendly implementation
...
Planned extensions of the \pkg{drtmle} package include extensions to
...
The \pkg{drtmle} package also allows users to pass in their own estimated OR and
...
\citet{vdl:2014:ijb} proposed modified IPTW estimators that allow for adaptive
...
distribution with estimable variance. In fact, \citet{vdl:2014:ijb} proposed a
...
methods are implemented for the `iptw_tmle()` estimator of \citet{vdl:2014:ijb}.
We conjecture that this estimator will have improved finite-sample behavior
...
achieving greater stability and efficiency. See \citet{ju2020robust} for further
discussion.
...
and `Q1W`. This may be relevant for users writing their own \pkg{SuperLearner}
