...
abstract: >
  ...
  However, in high-dimensional settings maximum likelihood estimation often breaks 
  down and standard procedures no longer yield correct inference. Instead, we may 
  rely on adaptive estimators of nuisance parameters to construct flexible regression estimators.
  ...
  inconsistent. \pkg{drtmle} implements
...
correspond to a drug, a potentially harmful exposure, or a policy
intervention. Often, the treatment cannot be randomized due to ethical or
...
distributions with estimable variance. Estimates of the asymptotic variance then
...
OR, paving the way for inference that is doubly-robust. \citet{benkeser:2017:biometrika}
...
\citet{benkeser:2017:biometrika}, as well as several extensions including cross-validated
...
these parameters have interpretations as mean counterfactual outcomes
...
finite samples [@porter:2011:ijb]. We refer readers to \citet{tlbook:2011}
for more about TMLE. 
...
construct doubly-robust confidence intervals and hypothesis tests. \citet{benkeser:2017:biometrika} proposed simplifications of the \citet{vdl:2014:ijb} estimators and
...
\citet{benkeser:2017:biometrika} demonstrated alternative equations that can be satisfied
...
both univariate regressions, while R-PS is a bivariate regression. \citet{benkeser:2017:biometrika} suggested that this may make R-PS1 and R-PS2 easier to estimate
...
doubly-robust limiting distribution. \citet{benkeser:2017:biometrika} provided a TMLE
...
(for discussion, see \citet{benkeser:2017:biometrika} Section 2.2). Recall that a TMLE
...
additional equations with the AIPTW estimator. \citet{benkeser:2017:biometrika} showed
...
improved estimators of \citet{benkeser:2017:biometrika} (`reduction = "univariate"`). The
...
Missing data are allowed in `A` and `Y`, as discussed below.
Beyond the data arguments, it is necessary to specify how to estimate
...
We refer interested readers to \citet{benkeser:2017:biometrika} for a theoretical
...
as required by `glm`. For the PS formula specified by `glm_g`, `colnames(W)` are 
available to use in the formula. For the OR (`glm_Q`) available names
of variables to be used in the formula depends on the `stratify` option.
If `stratify = FALSE`, a single OR is fit to
estimate $\bar{Q}_0(A,W)$. In this case, the OR regression is fit passing a `data.frame` into the `data` argument of `glm` that contains `W` and `A`. Thus, both `colnames(W)` and `"A"` may appear in the formula `glm_Q`. If `stratify = TRUE`, regressions are fit stratified by `A` to separately 
estimate $\bar{Q}_0(1,W)$ and $\bar{Q}_0(0,W)$. In this case, the `glm_Q` option 
may not include `"A"`.

To understand the variable names available for R-OR, recall that the parameter 
involves a regression onto the estimated propensity. Thus, the formula that is 
input via `glm_Qr` should
assume a data with a single column, `"gn"`. 
Similarly, the R-PS1 and R-PS2
that are computed for the estimators of \citet{benkeser:2017:biometrika} (the default
...
instead, the estimators of \citet{vdl:2014:ijb} are preferred (`reduction = "bivariate"`) the 
`glm_gr` formula assumes data with two columns, `"Qn"` and `"gn"`.
...
We illustrate non-stratified regression for both the univariate (default) and bivariate
reductions.
...
glm_fit_biv <- drtmle(W = W, A = A, Y = Y, family = binomial(),
                      glm_g = "W1 + W2", glm_Q = "W1 + W2*A",
                      glm_gr = "Qn", glm_Qr = "gn + Qn", stratify = FALSE,
                      reduction = "bivariate")
glm_fit_biv
```
...
(`SL.glm`) and adaptive regression splines (`SL.earth`, which utilizes the `earth`
...
analysis the results are dependent on the random seed that is set. To remove 
this dependence, one can average results over repeated super learner runs; more 
...
are *partially cross-validated* and do not require any additional fitting beyond a standard super learner. 
Here we describe briefly how these estimates are obtained and show calls to `drtmle` to obtain these estimates. 
...
to be $\bar{Q}_0(a,w) := E_0(\Delta_Y Y \mid \Delta_A A = a, \Delta_A = 1,
...
collaborative targeted maximum likelihood estimator (CTMLE) of \citet{vdl:2014:ijb} 
that also facilitates collaborative doubly-robust inference.
