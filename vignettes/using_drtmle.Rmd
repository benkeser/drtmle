...
Cross validation may significantly increase computation time necessary for running `drtmle`. Parallelized regression fitting is available by setting `parallel = TRUE`, which allows for use of the `foreach` package [@foreachPackage] to parallelize the estimation of the OR, PS, and reduced dimension regressions. The function `foreach` assumes that a a parallel back-end is registered before running `drtmle`. The appropriate back-end is system-specific, but below we demonstrate registration with a back-end based on the `doMC` package [@doMCPackage].
