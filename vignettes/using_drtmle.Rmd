...
option `parallel = TRUE`, which allows for use of the `foreach` package
[@foreachPackage], alongside the `future` and `doFuture` packages [], to
parallelize the estimation of the OR, PS, and reduced-dimension regressions. The
function `foreach` assumes that a parallel back-end is registered before running
`drtmle`. The appropriate back-end is system-specific, but below we demonstrate
registration with a back-end based on the `doParallel` [@doParallelPackage] and
`snow` packages [@snowPackage].
