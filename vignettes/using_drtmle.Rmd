...
option amounts only to parallelizing with futures across a standard foreach loop
structure, which is accomplished by combining the `foreach`, `future`, and
`doFuture` R packages [@foreachPackage, @futurePackage, @dofuturePackage].
Selecting this option results in parallelization of the estimation of the OR,
...
R processes (if the OS allows -- i.e., non-Windows systems) or spawns and uses
...
of back-end is completely flexible, and submission of the job to a wide variety
