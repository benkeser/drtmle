...
#' 
#' @importFrom stats lm nlm coef sd
#' 
#' @export
...
m.biasreducedDR.identity<-function(R,Y,cov){
  ...
  init.gamma<-stats::coef(stats::glm(R~cov,family="binomial"))
  sol<-stats::nlm(min.Uint,init.gamma)
  gamma.BR<-sol$estimate
  weight<-as.vector(1/exp(gamma.BR%*%t(int.cov)))
  beta.BR<-stats::coef(stats::lm(Y ~ -1+int.cov,subset=(R==1),weights=weight))
  mn.Y<-mean(U(R,Y,int.cov,gamma.BR,beta.BR))
  se.mn.Y<-stats::sd(U(R,Y,int.cov,gamma.BR,beta.BR))/sqrt(n)
