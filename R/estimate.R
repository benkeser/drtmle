...
estimateG <- function(A, W, DeltaY, DeltaA, SL_g, glm_g, a_0, tolg, 
  ...
  if(!all(DeltaA == 1)){
    ...
    if(!is.null(glm_g)){
      ...
      fm_DeltaA <- stats::glm(stats::as.formula(paste0("DeltaA~",glm_g$DeltaA)), 
  ...
  if(!is.null(SL_g)){
    ...
    if(length(SL_g$A)>1 | is.list(SL_g$A)){
      ...
      if(length(a_0)==length(unique(A)) & length(unique(A[!is.na(A)]))==2){
        fm_A <- SuperLearner::SuperLearner(Y = as.numeric(trainA[trainDeltaA==1]==a_0[1]), 
                                           X = trainW[trainDeltaA==1,,drop = FALSE], 
                                           newX = validW, 
                                           family = stats::binomial(),
                                           SL.library = SL_g$A,verbose=verbose,
                                           method = "method.CC_nloglik")
        ...
        for(a in a_0[1:(length(a_0)-1)]){
          ...
          if(a_ct != 0){
            gn_A[[a_ct + 1]] <- tmp_pred * 
                    Reduce("*",lapply(gn_A[1:a_ct], function(x){ 1 - x }))
          }else{
            gn_A[[a_ct + 1]] <- tmp_pred
          }
          fm_A[[a_ct + 1]] <- tmp_fm
      ...
      if(length(a_0)==length(unique(A[!is.na(A)])) & length(unique(A[!is.na(A)]))==2){
        gn_A <- vector(mode = "list", length = 2)
        fm_A <- do.call(SL_g$A, args=list(
                      Y=as.numeric(trainA[trainDeltaA==1]==a_0[1]), 
                      X=trainW[trainDeltaA==1,,drop=FALSE], 
                      newX=validW, obsWeights=rep(1,length(trainA[trainDeltaA==1])),
                      family=stats::binomial()
                      ))
  ...
  if(!is.null(glm_g)){
    if(length(a_0)==length(unique(A)) & length(unique(A[!is.na(A)]))==2){
      thisDat <- data.frame(A = as.numeric(trainA[trainDeltaA==1]==a_0[1]), 
                            trainW[trainDeltaA==1,,drop=FALSE])
  ...
  if(!all(DeltaY == 1)){
      ...
      if(length(SL_g$DeltaY) > 1 | is.list(SL_g$DeltaY)){
        ...
        # levels of A
          ...
          fm_DeltaY <- SuperLearner::SuperLearner(
                ...
                X=data.frame(A = trainA[include], trainW[include,,drop=FALSE]), 
                family=stats::binomial(),
                SL.library=SL_g$DeltaY,verbose=verbose,
                method = "method.CC_nloglik"
          )
          ...
          for(a in a_0){
            a_ct <- a_ct + 1
            gn_DeltaY[[a_ct]] <- stats::predict(fm_DeltaY,onlySL = TRUE,
               newdata = data.frame(A = a, validW))$pred
          }
        } # end if !stratify
      # if SL_g$DeltaY only a single algorithm, then call that directly
        ...
        }else{ # end if stratify
          # call algorithm to fit pooled estimate
    ...
    if(!is.null(glm_g)){
      ...
      }else{ # end stratified glm
        # fit glm in everyone with DeltaA == 1
        fm_DeltaY <- stats::glm(  
            stats::as.formula(paste0("trainDeltaY[include]~",glm_g$DeltaY)), 
            data=data.frame(A = trainA[include], 
                            trainW[include,,drop = FALSE]), 
            family=stats::binomial()
        )
        name_DeltaY <- paste0("DeltaY ~ W + A | DeltaA == 1")
  ...
  #------------------------------------------------------
  gn <- mapply(gn_A = gn_A, gn_DeltaY = gn_DeltaY, 
...
#' @importFrom stats predict glm as.formula
#' 
estimateQ <- function(Y, A, W, DeltaA, DeltaY, 
                      SL_Q, glm_Q, a_0, stratify, family,
                      verbose=FALSE, returnModels=FALSE,
                      validRows=NULL, ...){
     ...
     if(length(SL_Q) > 1 | is.list(SL_Q)){
      ...
      fm <- lapply(tmp, "[[", 2)
       ...
       tmp <- plyr::alply(a_0, 1, function(x){
        ...
        include2[is.na(include2)] <- FALSE
        # call function
         fm <- do.call(SL_Q, args = list(Y=trainY[include2 & include], 
                                         X=trainW[include2 & include,,drop=FALSE], 
                                         newX=validW,
                                         verbose=verbose,
  ...
  if(!is.null(glm_Q)){
    if(!stratify){
      fm <- stats::glm(stats::as.formula(paste0("Y~", glm_Q)), 
                       data = data.frame(Y = trainY, A = trainA, 
                                         trainW)[include,,drop=FALSE], 
                       family = family)
      Qn <- plyr::alply(matrix(a_0), 1, function(a,fm){
        stats::predict(fm, newdata=data.frame(A = a, validW), type="response")
      },fm=fm)
    }else{
      tmp <- plyr::alply(matrix(a_0), 1, function(a){
        ...
        # handle NAs properly
        include2[is.na(include2)] <- FALSE
        fm <- stats::glm(stats::as.formula(
                            paste0("trainY[include2 & include] ~ ", glm_Q)), 
...
estimateQrn  <- function(Y, A, W, DeltaA, DeltaY,
  ...
  if(!is.null(SL_Qr)){
    Qrn <- mapply(a = a_0, train_g = train_gn, train_Q = train_Qn, 
      ...
      Aeq_a <- trainA == a; Aeq_a[is.na(Aeq_a)] <- FALSE
  ...
  return(list(est = lapply(Qrn,function(x){ x$est }), 
              fm = lapply(Qrn,function(x){ fm = x$fm })))
  Qrn 
}
...
estimategrn <- function(Y, A, W, DeltaA, DeltaY, 
                        Qn, gn, SL_gr, tolg, 
    ...
    grn <- mapply(a = a_0, train_Q = train_Qn, train_g = train_gn,
      ...
      if(length(unique(train_Q)) == 1){
        ...
        if(reduction == "univariate"){
          m1 <- mean((as.numeric(
                        Aeq_a & trainDeltaA == 1 & trainDeltaY == 1
                      ) - train_g)/train_g)
        ...
        if(length(SL_gr) > 1){
          if(reduction == "univariate"){
            fm1 <- SuperLearner::SuperLearner(
                    Y = (as.numeric(Aeq_a & trainDeltaA == 1 & trainDeltaY == 1) - 
                          train_g)/train_g, 
  ...
  if(!is.null(glm_gr)){
    grn <- mapply(a = a_0, train_Q = train_Qn, train_g = train_gn,
      ...
      if(reduction == "univariate"){
        fm1 <- stats::glm(
                stats::as.formula(paste0("grn1~", glm_gr)), family = "gaussian",
                data=data.frame(
                      grn1 = (as.numeric(Aeq_a & trainDeltaA == 1 & trainDeltaY == 1) - 
                               train_g)/train_g, 
                      Qn = train_Q))
        grn1 <- stats::predict(fm1, newdata = data.frame(
                  grn1 = rep(0, length(validA)), # dummy placeholder
                  Qn = valid_Q), type="response")
        fm2 <- stats::glm(
                stats::as.formula(paste0("A~",glm_gr)), family = "binomial",
                data=data.frame(
                  A=as.numeric(Aeq_a & trainDeltaY == 1 & trainDeltaA == 1), 
                  Qn = train_Q))
